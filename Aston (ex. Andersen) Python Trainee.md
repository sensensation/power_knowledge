# by cybersen | www.linkedin.com/rodkir
...
9 - int, bool, float, str, bytes, bytearray, complex, Decimal, NoneType - базовые типы данных в Python
	...
	\.
	complex - Комплексные числа -  numbers.Complex
	...
	\.
10 - Обработка ошибок. Exceptions
	...
	Python изнутри:
		...
		**Интерпретация** - это чтение файла с исходным кодом программы и поэтапное ее выполнение
		...
		import <mark style="background: #BBFABBA6;">gc</mark>:
			...
			\.
	\.
12 - Collections: list, tuple, set, frozenset, dict, defaultdict
	Коллекции:
	...
	<mark style="background: #FFB86CA6;">dict</mark> & <mark style="background: #FFB86CA6;">defaultdict</mark> & <mark style="background: #FFB86CA6;">OrderedDict</mark> - ассоциативный массив - изменяемые отображения ссылок на объекты, доступные по ключу. <mark style="background: #CACFD9A6;">{None: "1", type(None): "2"}</mark> два разных объектов
		...
		\.
		<mark style="background: #FFB86CA6;">OrderedDict from collections</mark> - упорядоченный словарь
			...
			my_dict = OrderedDict()
	\.
13 - Annotations + Dataclasses, namedtuple, Enum
...
14 - MRO | Method Resolution Order
	...
	Ты super()!
		**super()** – функция, позволяющая явно ссылаться на родительский класс.
		...
		\.
15 - Scopes & Namespaces (local, nonlocal, global) 
	Scopes - области имен
	...
	\.
16 - Double underscore methods | Descriptors | Python Data Model | @property
		...
		\.
17 - Generators. Context manager. Comprehensions
	<mark style="background: #ABF7F7A6;">Генераторы</mark>:
		...
		\.
		<mark style="background: #ADCCFFA6;">Итератор</mark> - это сущность порождаемая функцией `iter`, с помощью которой происходит итерирование **итерируемого объекта**.
		\.
...
18 - Decorators
    ...
    Иерархия декораторов:
		...
		def foo():
		    pass
	Отработает как -> dec3(dec2(dec1(foo()))) (снизу вверх)
		@dec3(sigma)
		@dec2(betta)
		@dec1(alpha)
...
21 - Абстрактные классы | abc
	...
	Комбинирование докароторов с @abstractmethod:
		...
		def from_dict(cls, d):
			. . .
		\.
22 - Lambda functions | throw-away fucntion
	...
	Аннотирование lambda:
		...
		Формат будет такой: два числа что берет, одно число что возвращает
				`typing.Callable[[int, int], int]
		\.
23 - Вложенные функции и замыкание | Рекурсия
	Рекурсия
	...
	Вложенность и замыкание
		 ...
		 Можно ли создать inner function внутри inner function:
			...
			   return inner()
		\.
24 - GIL | Global Interpreter Lock
	GIL -
		**Python Global Interpreter Lock (GIL)** — это блокировка, позволяющая только одному потоку управлять интерпретатором Python. Это означает, что будет выполняться только один поток в рамках главного процесса.
	    ...
	    Альтернатива GIL:
			...
			\.
25 - Threading | Многопоточность | asyncio | Асинхронность
	...
	Работа с <mark style="background: #FF5582A6;">threading</mark>
		...
		Коротко про Barrier, Semaphore, Timer, Event:
			- <mark style="background: #FFF3A3A6;">Semaphore()</mark> - семафор использует внутренний счетчик, который уменьшается при каждом вызове acquire() и увеличивается при release().  Счётчик не может стать меньше нуля, когда он становится равным нулём, acquire() блокирует поток.
	...
	Работа с <mark style="background: #FF5582A6;">asyncio</mark>:
		...
		Блокирующие функции:
			...
			\.
26 - Multiprocessing | Параллелизм
	...
	Работа с multiprocessing:
		...
		\.
27 - Filesystem Paths | os | pathlib 
...
28 - datetime
	...
	Как проводить операции над aware & naive объектами:
		...
		    return diff
		 \.
29 - I/O Objects | BytesIO | TextIO | open
	io
		...
		\.
30 - Уровни тестирования и теория
...
32 - SQL
	...
	<mark style="background: #ABF7F7A6;">Индексы SQL</mark>:
		Индекс – <mark style="background: #ADCCFFA6;">это структура данных, ускоряющая выборку данных из таблицы за счет дополнительных операций записи и дополнительного пространства на диске</mark>, используемых для хранения структуры данных и поддержания её в актуальном состоянии.
		\.
	...
	\.
	<mark style="background: #ABF7F7A6;">Уровни изоляции</mark>:
...
33 - websocket
	<mark style="background: #ABF7F7A6;">Модуль socket</mark> \.
		...
		\.
		Проверка состояния реального времени проверяется через ping-pong
		\.
...
34 - web api  REST | SOAP
	...
	<mark style="background: #ABF7F7A6;">SOAP</mark>:
		...
		\.
35 - Django Rest Framework | ASGI | WSGI
